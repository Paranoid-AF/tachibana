...
const _isCompiled = import.meta.dirname?.startsWith('/$bunfs/') ?? false
...
export function getNative(): NativeAddon {
  ...
  const distDir = _isCompiled
    ? join(dirname(process.execPath), 'ios-connect', 'dist')
    : join(dirname(import.meta.dirname!), '..', 'dist')
  _native = _require(join(distDir, 'index.js')) as NativeAddon
  return _native
}
