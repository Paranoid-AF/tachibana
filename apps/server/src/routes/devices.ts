...
import { getDeviceMeta, saveDeviceMeta, getDevicePrefs, setDevicePrefs } from '../libs/device-store.ts'
...
export const deviceRoutes = new Elysia({ prefix: '/devices' })
  ...
  .get('/:udid/apps', async ({ params, set }) => {
    ...
    const filtered = allApps.filter(app =>
      !wdaBundleId || !app.bundleId.startsWith(wdaBundleId),
    )
    ...
    return settled
      .filter((r): r is PromiseFulfilledResult<{ bundleId: string; name: string; iconUrl60?: string; iconUrl100?: string }> => r.status === 'fulfilled')
      .map(r => r.value)
  })
